...
def gebaeudereferenzen_einlesen():
    ...
    for line in F:
        line = line.strip()
        m = re.split(';', line)
        punkt = "POINT(" + m[11].replace(",", ".") + " " + m[12].replace(",", ".") + ")"
        SQL = """INSERT INTO public.gebref(\"NBA\", \"OI\", qualitaet, land, regbez, kreis, gemeinde, gdeteil, strasse, hnr,
                zusatz, rechts, hoch, strassenname,adresse)
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s, %s)"""
        data = (m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7], m[8], m[9], m[10], m[11], m[12], m[13], (m[13] + " " +
                                                                                                         m[9] + m[
                                                                                                             10]).strip())
        if i >= 0:
             cur.execute(SQL, data)
